...
**讲法要点——诚实、分层、可验证：**
1. **别说"AI 自动设计探针/自动判定"**——那是假的（规则仍是主判），深挖就穿帮。说"**规则驱动、AI 参谋、数据回测、人裁决**"。
2. 每个 agent 讲清楚"思考什么"：Planner 思考调优优先级；探针设计是"AI 审校 + 规则兜底 + 离线回测验证"；Judge 是"规则判三态 + AI 挑边界案例 + 人裁定"；Warden 思考检查有效性；Curator 从统计到解读。
3. 核心卖点是**可验证的反思闭环**：AI 提建议 → 规则兜底 → 数据回测 → 人裁定 → 反思校准。每一步都有日志、回测表、memory 反思区可查——这是真的，经得起深挖。
4. 术语建议："**参谋层（Advisor Layer）**" + "**反思环（Reflection Loop）**" + "**人在环（Human-in-the-loop）**"——准确、不吹牛、好追问。

**准备好被深挖的问题：**
- "探针是 AI 设计的吗？" → 规则反推法生成，AI 审校提改进建议，Warden 兜底，离线 A/B 回测证明建议是否有效（probe_hitrate.json 可查）。
- "判定是 AI 定的吗？" → 不是。规则判三态，AI 只在灰色地带挑边界给人看，人最终裁定（有 L152/L153 案例背书）。
- "AI 会改你的规则吗？" → 不会。反思产出建议，rules.json 改动必须人确认，是单一真源。
- "怎么证明 AI 有用？" → 探针命中率回测表：脚本基线 vs 脚本+AI 审校的离线 A/B。
