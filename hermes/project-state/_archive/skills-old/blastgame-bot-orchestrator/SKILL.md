...
```bash
python "scripts/submit_batch_unity.py" "89,90,91" --games 400 --tiers "1,2,3,4,5"
```

**提交前运行 `preflight.py` 验证：**
```bash
python tools/preflight.py submit --levels 89,90,91 --tiers 1,2,3,4,5
```
...
**监控工具：** `python tools/monitor_bot.py "81" --tiers "1,2,3,4,5" --timeout 7200`
...
### 批后分析（新批次结果对比池子）
```bash
python tools/post_batch_review.py           # 最新 batch
python tools/post_batch_review.py --batch "82_98-..."  # 指定 batch
python tools/post_batch_review.py 59,81 --full  # 带死亡分布
```
...
| **Tier 映射错位**：T1 配置实际在 T5 档跑 | **根因已定位（2026-07-20）：** AssetDatabase 缓存导致的读取错乱。最终修复在 `BlastBotJenkinsBatchEntry.RunFromCommandLine` 加 `AssetDatabase.Refresh()`。排查时先确认 `probe_configs.json` 的值是否覆盖了 asset。 |
