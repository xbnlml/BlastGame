...
metadata:
  hermes:
    ...
    related_skills: [blastgame-multi-tier-designer, blastgame-bot-orchestrator, blastgame-judgment]
---

# BlastGame 关卡调优

> 工作目录：`D:\\download\\BlastGame\\hermes\\`。游戏工程：`$BLASTGAME_REPO`。
...
### 批C — 裁定（批跑完成后立即执行，不等用户问）
1. **立即：** `python tools/post_batch_review.py --batch {batch_dir} --full`（批后自动分析：展示新数据 vs 之前 best 的 WR/死亡分布变化，并逐档检查 probe configs 是否与预期一致）
...
# 提交（默认 scoring_opt_vg 策略）
python scripts/submit_batch_unity.py "56,57,71" --games 400 --tiers "1,2,3,4,5" --strategy scoring_opt_vg

# 批跑完成后，自动执行：
# 1. dump_level_pools.py 刷新池子
# 2. post_batch_review.py --batch <目录> --full 做监控分析
# 以上两步在 submit_batch_unity.py 中已自动完成
...
> Cron watchdog、Unity 守护、管道阶段演进。只在全自动批处理模式使用。
...
| 阶段 | Bot 30min | Unity | request.json | 处理 |
|------|----------|-------|-------------|------|
| running 🟢 | ✅ | ✅ | 消费中 | 正常 |
| stalled 🟡 | ❌ | ✅/❌ | 无/stale | 通知→[SILENT] |
| dead 🔴 | ❌ | ❌ | 存在+陈旧 | 重启Unity |

### Unity 守护

**`ps aux` 在 git-bash 下假阴性，必须用 `tasklist`：**
```bash
...
|| 18 | **tier 映射 bug** — 多轮提交 T1 读到的配置与 T4/T5 相同。**根因已确认：AssetDatabase 缓存过期 + submit_batch_unity.py preflight 覆盖 asset。** Python 写 asset 后 Unity 的旧二进制缓存使 DynamicDifficultyConfigs 读取错序，同时 submit 脚本的 preflight 会覆盖手动写入。排查时先查 `probe_configs.json` 再查 `AssetDatabase.Refresh()`。 | 排查写入问题先查 `probe_configs.json` |
|| 19 | **Normal 模式重复档位浪费** — Normal 关 T1=T2、T4=T5 同配置，Unity 内 `BuildBotBatchDedupeKey` 会检测到相同 sd/sc/ratios/of 并跳过重复档位直接克隆结果。直接 `--tiers "1,2,3,4,5"` 即可，不需要分两次跑。 | 用 --tiers 1,2,3,4,5 省事 |
|| 20 | **`submit_batch_unity.py --tiers` 可指定任意档位** — 不需要跑全部5档。Hard/SuperHard 用 `--tiers 1,2,3,4,5`，Normal 用 `--tiers 1,2,3,4,5`（Unity自动跳过重复）或 `--tiers 1,3,5`（Python层面节省写asset时间）。 | 灵活用 --tiers |
