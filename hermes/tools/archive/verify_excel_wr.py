...
EXCEL = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), '手动挑配置记录.xlsx')
STAGE = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'stage-data')
LEVELS_51_200 = [str(lv) for lv in range(51, 201)]
...
def main():
    ...
    for r in range(2, ws.max_row + 1):
        ...
        lv = prev_lv
        if not lv or lv not in LEVELS_51_200:
            continue
