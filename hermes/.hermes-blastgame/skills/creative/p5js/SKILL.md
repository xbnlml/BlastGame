...
| `troubleshooting.md` | Performance profiling, per-pixel budgets, common mistakes, browser compatibility, WebGL debugging, font loading issues, pixel density traps, memory leaks, CORS |
| `references/pretext-patterns.md` | Pretext community demo patterns: reflow, text-as-geometry, kinetic type, editorial, shrink-wrap |
...
| `templates/pretext-hello-orb.html` | Pretext demo: text reflowing around a moving orb (reflow-around-obstacle pattern) |
| `templates/pretext-donut-orbit.html` | Pretext demo: advanced measured ASCII obstacle layout with draggable shapes |

---

## Bonus: Text-Based Creative Coding with @chenglou/pretext

For text-powered demos (kinetic typography, text reflowing around obstacles, ASCII games made of real prose), use the `@chenglou/pretext` library — a 15KB zero-dependency TypeScript tool for DOM-free multiline text measurement.

**When to use:** The user wants ASCII art from real words (not monospace rasters), text flowing around moving sprites, kinetic typography with per-glyph physics, or text-as-geometry games.

**Import pattern:**
```html
<script type="module">
import { prepareWithSegments, layoutWithLines, layoutNextLineRange, materializeLineRange }
  from "https://esm.sh/@chenglou/pretext@0.0.6";
</script>
```

**Two key use cases:**
- Fixed-width layout: `prepare → layoutWithLines → fillText lines`
- Variable-width (obstacle-aware): `prepare → layoutNextLineRange in a y-loop with per-row width`

**Templates:** `templates/pretext-hello-orb.html` (simple reflow) and `templates/pretext-donut-orbit.html` (advanced ASCII obstacles with draggable geometry).

**Palette hint:** Pretext's vibe is "not monospaced." Use proportional fonts (Inter, Iowan Old Style, Georgia). Dark backgrounds, real prose (never lorem ipsum). For detailed community demo references and full pitfall catalog, load the `pretext` skill via `skill_view("pretext")`.
