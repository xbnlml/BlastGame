...
For finding reaction GIFs and media from the Tenor API, see `references/gif-search.md`. Quick pattern:

```bash
curl -s "https://tenor.googleapis.com/v2/search?q=thumbs+up&key=${TENOR_API_KEY}&limit=5" | jq -r '.results[].media_formats.gif.url'
```

Requires `TENOR_API_KEY` in your `.env` (free from Google Cloud Console). The reference covers search, download, metadata extraction, and all API parameters.
