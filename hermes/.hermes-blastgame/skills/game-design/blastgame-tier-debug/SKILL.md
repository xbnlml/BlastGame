...
asset 文件在 `test/` 和 `funnel_b/` 各有一份。**Python 工具只写 `test/`。绝不写 funnel_b/ 下的文件。**

**重要：`funnel_b/` 是竞品关卡数据，不是原始设计。** 和 `test/` 完全不是一套东西。
- 同编号关卡在两个文件夹下是不同游戏的 level，只是编号碰巧一样
- **绝对不能用 funnel_b 的数据修复 test/ 的文件**
- 修复 test/ 文件损坏时参考源顺序：备份 > test 自身结构 >（不要用 funnel_b）

```bash
diff "$BLASTGAME_REPO/Assets/.../test/{LV}.asset" "$BLASTGAME_REPO/Assets/.../funnel_b/{LV}.asset"
```

如果不同，这是正常的——`funnel_b/` 是竞品数据，`test/` 是我们自己设计的关卡。**不能在 Python 侧写 funnel_b/，也不能用 funnel_b 当参考源。**
...
- 双写 test/ + funnel_b/ — 污染用户游戏的默认配置，已被严厉批评。
- **修改任何 `$BLASTGAME_REPO` 下文件都被禁止**。包括 .cs、.asset（funnel_b/）、Library、Temp。所有 workaround 只能限制在 Hermes 目录内。
...
### 严禁修改 BlastGame 项目文件
绝不写 `$BLASTGAME_REPO` 下的任何文件（.cs、.asset、funnel_b/、Library、Temp）。所有排查/修复限制在 Hermes 目录内。**funnel_b/ 的 asset 绝对不能碰——那是用户游戏的默认配置池。**
